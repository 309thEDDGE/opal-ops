    ...
    def test_generate_docker_compose(self,contextData):
        expected = {'version': '3.9', 
                    'services': {
                        ...
                        'keycloak': {'image': '${KEYCLOAK_IMAGE}', 'command': 'start', 'depends_on': {'postgresql': {'condition': 'service_healthy'}}, 'volumes': ['./keycloak/certs/test_context/tls.key:/etc/x509/https/tls.key', './keycloak/certs/test_context/tls.crt:/etc/x509/https/tls.crt',"keycloak_log_storage:/logs"], 'env_file': ['./.env.secrets', './.test_context.env', './.env'], 'healthcheck': {'test': ['CMD-SHELL', 'curl --fail http://localhost:9000/health'], 'interval': '60s', 'timeout': '5s', 'start_period': '60s', 'retries': 10}, 'labels': ['traefik.enable=true', 'traefik.http.routers.keycloak.rule=Host(`keycloak`)', 'traefik.http.routers.keycloak.entrypoints=websecure', 'traefik.http.services.keycloak.loadbalancer.server.port=8080', 'traefik.http.routers.keycloak.service=keycloak'], 'restart': 'always'},
